...
//        mHelpLiveo.add(getString(R.string.spam), R.mipmap.ic_report_black_24dp, 120);
    ...
    public void SDKinitializations() {
        ...
        //Localytics.setCustomerId(LoginActivity.android_id);
        ...
        Token setUpToken = new Token(LoginActivity.registrationId, LoginActivity.android_id, "android");
        boolean foundToken = false;

        for (int i = 0; i < LoginActivity.allTokens.size(); i++){
            Log.i("MyActivity", "Token: " + LoginActivity.allTokens.get(i).getToken());
            if (LoginActivity.allTokens.get(i).getToken().equals(LoginActivity.registrationId)){
                foundToken = true;
            }
        }

        if (!foundToken){
            Firebase sendTokenToServer = new Firebase(LoginActivity.useFirebase+"Users/PushToken");
        ...
        if (!feedbackMessage.getText().toString().equals("")) {
            sendFeedback.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    uploadFeedback = new Firebase(LoginActivity.useFirebase+"Feedback/" + currentDateandTime);
                    uploadFeedback.child("timestamp").setValue(currentDateandTime);
                    uploadFeedback.child("userId").setValue(LoginActivity.android_id);
