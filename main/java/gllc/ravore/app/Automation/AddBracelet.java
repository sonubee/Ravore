...
public class AddBracelet {
    ...
    public AddBracelet(String giverOrReceiver, String braceletId, Context context, String fromLoginOrMain, GoToMainActivity goToMainActivity) {
        ...
        if (LoginActivity.allBracelets.size() > 2){
            if (braceletId.equals("")) {
                ...
                for (int i = 0; i < LoginActivity.allBracelets.size(); i++) {
                    if (LoginActivity.allBracelets.get(i).getBraceletId().equals(braceletId)) {
                        bracelet = LoginActivity.allBracelets.get(i);
                        found = true;
                    }
                }

                String dateString = new SimpleDateFormat("MM" + "/" + "dd" + "/" + "yyyy").format(new Date());
                String key = LoginActivity.braceletKey.get(bracelet.getBraceletId());
                Firebase updateBracelet = new Firebase(LoginActivity.useFirebase + "Bracelets/" + key);
                ...
                if (found) {
                    if (giverOrReceiver.equals("giver")) {
                        if (bracelet.getGiverId().equals("NA")) {
                            ...
                            newGiverMap.put("giverId", LoginActivity.android_id);
                        ...
                        } else if (bracelet.getGiverId().equals(LoginActivity.android_id)) {
                        ...
                        if (bracelet.getGiverId().equals(LoginActivity.android_id)) {
                            ...
                            if (bracelet.getReceiverId().equals("NA") && !bracelet.getGiverId().equals("NA")) {
                                ...
                                Map<String, Object> newGiverMap = new HashMap<>();
                                newGiverMap.put("receiverId", LoginActivity.android_id);
                                newGiverMap.put("dateReceived", dateString);
                                updateBracelet.updateChildren(newGiverMap);

                                for (int j = 0; j < LoginActivity.allTokens.size(); j++) {
                                    if (bracelet.getGiverId().equals(LoginActivity.allTokens.get(j).getUserId())) {
                                        new SendPush("Bracelet " + bracelet.getBraceletId() + " has been added!", LoginActivity.allTokens.get(j).getToken(), "Bracelet Added!", "addition", bracelet.getBraceletId(), LoginActivity.allTokens.get(j).getOs());
                                    }
                                }
